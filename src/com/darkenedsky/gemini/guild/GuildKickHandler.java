...
public class GuildKickHandler extends Handler { 
		...
		public void processMessage(Message e, Player p) throws Exception {
			...
			// get the player's rank - first let's try to get lucky and see if they're online			
			try { 
				kicked = service.getSessionManager().getSession(playerid);
			...
			// they weren't online; get it from the DB
				...
				kicked = service.getSessionManager().loadPlayerFromDatabase(playerid);
			}
			...
			// ok, toss him out
			PreparedStatement ps = service.getJDBC().prepareStatement("update players set guildid = null, guildrank = null where playerid = ?;");
